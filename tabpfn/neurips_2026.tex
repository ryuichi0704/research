...
\begin{abstract}
PFNs are pretrained on synthetic task libraries, so deployment robustness is a prior-design problem. We argue that the central question is not merely whether to mix broadly, but which family distinctions are safe to collapse. Broad mixtures hedge deployment uncertainty, yet in rare-event boundary regimes collapsing across mismatched families can change the one-step predictive tail law. We first give a minimal oracle split/merge calculus: among prompt-only predictors the collapsed predictive is Bayes-optimal, and the irreducible price of hiding family structure is the prompt-conditioned predictive distinctiveness $\I(F;Y\mid O)$, equivalently a generalized Jensen--Shannon divergence between family predictives after the prompt. Bucket refinement gains exactly the conditional mutual information it reveals, while practical bucketed PFNs trade router cross-entropy against within-bucket expert KL. We then prove a sharp boundary theory. In Beta--Bernoulli tasks, collapsed mixtures inherit the most singular boundary component, fixed-count mismatches already force the slow $n^{-(1+c)}$ law, and matched boundary profiles restore the regular $n^{-2}$ regime. A regular-variation extension shows that this is a general rare-event phenomenon, and zero-count slopes provide an operational diagnostic for boundary-aware bucketing. Finally, we formalize class balancing as a PFN-specific prompt intervention: any prompt transformation incurs exactly the predictive information it deletes, so hiding original class-count statistics has an irreducible Bayes cost; in the canonical Beta--Bernoulli prevalence model, a perfectly balanced prompt is asymptotically no more informative about prevalence than no prompt at all; and a minimal case-control aliasing construction shows that balanced prompts can induce query-dependent score distortions and even ranking reversals that no threshold shift can repair. Exact closed-form experiments verify the phase transition and the benefit of preserving shared-profile buckets. The resulting design rule is asymmetric: mix broadly for coverage, route across boundary profiles in rare-event regimes, and do not balance away the prevalence evidence that identifies those profiles.
\end{abstract}
...
Class imbalance is a particularly important stress test of that distinction. In ordinary supervised learning, label balancing is often framed as a loss-design or retrospective-sampling issue. For PFNs, the prompt itself is the inference object. Replacing a naturally sampled prompt by a $1{:}1$ case-control prompt therefore changes the observation channel seen by the oracle, potentially deleting exactly the prevalence evidence that identifies the latent family. We will show that this loss is measurable, irreducible at the oracle level, and not generally recoverable by threshold tuning alone. In the simplest Beta--Bernoulli prevalence model, perfect balancing is even stronger: it makes arbitrarily long prompts asymptotically useless for learning the task base rate.
...
\section{Class Imbalance as a Prompt-Intervention Problem}
\label{sec:imbalance}

Class imbalance raises a PFN-specific question not resolved by the boundary asymptotics alone. In ordinary supervised learning, label balancing is often analyzed as a retrospective sampling design, and recent TabPFN model reports confirm that threshold tuning can materially improve F1 on imbalanced benchmarks \citep[Fig.~17]{grinsztajn2025tabpfn25}. The oracle question, however, is sharper: if a PFN uses the prompt itself as evidence about the latent task, what happens when we replace the natural i.i.d. prompt by a label-balanced one? The answer is that balancing is not just a downstream threshold choice; it is a transformation of the observable sigma-field. Any deleted prompt statistic creates an irreducible Bayes penalty, and when the deleted statistic is the original prevalence count, the resulting error can be qualitatively different from the usual intercept-only case-control correction.
