...
BatchEnsemble design and records the technical fixed-$K$ width limit that
supports both specializations. The abstract mean-field setup, standing
assumptions, and all proofs are deferred to the appendix.
...
\begin{theorem}[Boundary-free wide-limit collapse]
\label{thm:boundaryfree-collapse}
Assume \cref{ass:main} and suppose the feature maps are independent of $h$.
Then the fixed-$K$ mean-field system reduces to the transport PDE
\begin{equation}
  \partial_t \rho_t
  +
...
\]
\smallskip
If, in addition, the empirical initial laws $\rho_0^M$ converge to a
deterministic law $\rho_0$ in probability, almost surely, or in $L^1$, and if
...
\[
  ...
  \forall \pi\in S_K,
\]
then the infinite-width predictor vector is deterministic and the unique
characteristic solution of \eqref{eq:pde-free} satisfies
\[
...
For a practitioner, the message is simple: once all boundary fast weights are
removed, the wide-limit theory predicts a single-model leading-order behavior.
Any head-to-head diversity that remains at finite width is therefore a
subleading fluctuation effect rather than an order-one property of the
...
The contrast with \cref{thm:boundaryfree-collapse} is structural. Without boundary fast
...
\begin{proof}[Proof of \cref{thm:boundaryfree-collapse}]
If $\Psi_\alpha$ does not depend on $h$, then $\nabla_{h_\alpha}\Psi_\alpha=0$
and hence $G\equiv 0$, which proves the reduction to \eqref{eq:pde-free}.
Since there is no boundary state left, \cref{cor:random} specializes to a
deterministic infinite-width limit whenever $\rho_0^M\to\rho_0$ in one of the
stated modes.

To prove collapse, define $\tilde\rho_t:=(\Pi_\pi^\Theta)_\#\rho_t$. Because the
feature maps do not depend on $h$, \eqref{eq:equiv} reduces to
\[
...
law. Uniqueness gives $\tilde\rho_t=\rho_t$, and choosing a permutation sending
any head index to any other one yields
\[
  f_1(x;\rho_t)=\cdots=f_K(x;\rho_t)
  \qquad
  \forall x\in\mathcal X,\ \forall t\ge 0.
