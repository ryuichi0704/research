...
\begin{abstract}
We study the practice-faithful $K=1$ Gaussian distributional head for a
two-layer network and connect finite-horizon mean-field training to linear mode
connectivity (LMC). The network predicts the first Gaussian natural coordinate
directly and a raw scale field passed through a softplus link, so the output
loss is convex in natural parameters but parameter interpolation is not the
same as natural-output interpolation.

Our training-side contribution is finite-horizon and distributional: for
noiseless SGD, two width-$N$ runs started from the same initialization law
track the same deterministic mean-field path on every fixed interval $[0,T]$.
This is a shared-path statement, not a long-time convergence theorem to a
stationary law. Our interpolation-side contribution is a deterministic aligned
barrier theorem in natural-parameter space. The barrier is controlled by two
explicit defects: a vector-head transfer term coming from hidden-weight mismatch
and a softplus Jensen term coming from the curvature of the reparameterization.
Combining these ingredients yields an LMC theorem for the practical
single-Gaussian head. Appendix~A records a complete continuous-time full-batch
analogue, and Appendix~B collects secondary rate refinements.
\end{abstract}
...
heteroscedastic Gaussian regression. The head is practice faithful but no
longer scalar. The network produces a two-coordinate raw output
\[
...
The single-Gaussian case is the clean place to begin. There is no mixture
softmax, no component permutation, and no gauge freedom, yet the head is still
genuinely vector valued. The Gaussian negative log-likelihood is exactly convex
in natural coordinates, but the practical softplus implementation breaks affine
interpolation in natural space. The interpolation problem therefore survives in
a simpler but still nontrivial form.
...
needed for the LMC bridge. For readability, the main text develops the SGD
route and the natural-parameter interpolation geometry. Appendix~A records a
complete continuous-time full-batch analogue, and Appendix~B collects rate
refinements.
...
\]
Whenever we discuss finite-horizon training dynamics, we assume the
initialization law $\rho_0$ on $\R^2\times\R^d$ has compact support.
...
This section contains the training-side contribution. The goal is finite-horizon
shared-path control in natural-parameter space: two wide runs started from the
same law track the same deterministic mean-field distributional trajectory on
every fixed interval $[0,T]$. We first record the model-specific drift and
confinement estimates, and then invoke the standard bounded-support SGD
mean-field machinery of Mei--Misiakiewicz--Montanari
\cite{MeiMisiakiewiczMontanari2019} as organized by
...
\begin{corollary}[Shared-law common path for SGD]
...
Appendix~A proves a complete continuous-time full-batch analogue built from the
same drift formula and confinement family. The main text now turns to the new
interpolation-side geometry in natural-parameter space.

\section{Interpolation Geometry in Natural-Parameter Space}

This section contains the interpolation-side contribution. Exact convexity
holds in natural-parameter space, so the only remaining gap between parameter
...
\section{From a Shared Mean-Field Path to Linear Mode Connectivity}

This section combines the shared-path input with the deterministic barrier
bound above. The alignment step follows the optimal-transport route of
Ferbach et al.~\cite{Ferbach2024}, while the new model-specific ingredient is
the natural-parameter barrier from Section~4.
...
Euclidean cost on $\R^{2+d}$. Then there exists a permutation $\pi_N$ such
that
\begin{equation}
...
For the practical single-Gaussian natural-softplus head, the mean-field-to-LMC
program separates cleanly into a training side and an interpolation side. On
the training side, finite-horizon SGD admits a shared-law common-path theorem:
two wide runs started from the same law track the same deterministic
distributional trajectory on every fixed interval $[0,T]$. On the interpolation
side, exact convexity in natural parameters reduces the barrier analysis to two
explicit defects, namely vector-head transfer and softplus curvature.

Combining these ingredients yields an LMC theorem for the practical $K=1$
distributional model. Appendix~A records a complete continuous-time full-batch
analogue, and Appendix~B collects rate refinements that are secondary to the
main theorem stack.

\appendix

\section{Continuous-Time Full-Batch Baseline}

This appendix records the complete continuous-time analogue of the training-side
bridge. It uses the same deterministic drift
\eqref{eq:drift-a-k1}--\eqref{eq:drift-xi-k1} and the same confinement family
from Proposition~\ref{prop:softplus-confinement-k1}. The bounded-support
well-posedness argument is standard in mean-field theory; see, for example,
Sirignano and Spiliopoulos~\cite{SirignanoSpiliopoulos2020}.

\begin{assumption}[Continuous-time full-batch regime]
\label{ass:complete-full-batch-k1}
Fix a horizon $T<\infty$. Training is continuous-time full-batch gradient flow
on the population risk \eqref{eq:population_risk}, with the usual mean-field
scaling and the practical natural-softplus head \eqref{eq:natural_head}.
\end{assumption}
...
\begin{enumerate}[label=(\roman*)]
  \item for every initial measure $\mu$ supported on $B_0$, the equation
  \eqref{eq:mf-pde-k1} admits a unique measure-valued path $(\rho_t)_{t\in[0,T]}$
  such that $\supp(\rho_t)\subseteq B_t$ for all $t\in[0,T]$;
