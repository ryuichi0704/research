...
On $\mathcal S_{M,\lambda}$ and for $|y|\le Y_\ast$, define
\begin{align}
...
Assume that, for every $t\in[0,1]$ and every $x\in\mathcal X$, both
...
\begin{proof}
Let $\tilde z_t:=(\hat u_t,\hat s_t)$ and $\tilde\eta_t:=\Gamma(\tilde z_t)$.
...
\[
  ...
  \big].
\]
Insert $\ell_\eta(\tilde\eta_t(X);Y)=\tilde\ell(\tilde z_t(X);Y)$ and bound the
first difference by the mean value theorem on $\mathcal S_{M,\lambda}$. The
second difference is exactly the softplus Jensen term, controlled by
\eqref{eq:C2}.
\end{proof}
...
$\delta_{u,t}(x)\le\delta_u^{\rm path}(x)$ and
...
For the timewise exact-modulus route, the instantiated decomposition is
\[
