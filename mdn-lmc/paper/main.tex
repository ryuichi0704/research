...
Linear mode connectivity (LMC) refers to the empirical phenomenon that trained
networks can often be connected by a low-loss path. Related empirical work, for
example by Garipov et al.~\cite{Garipov2018} and
Frankle et al.~\cite{Frankle2020}, documented simple low-loss connections and
linearly connected regions. In architectures with permutation symmetries, one
often studies such connections after an appropriate neuron permutation. In the
two-layer mean-field setting, Ferbach et al.~\cite{Ferbach2024} gave a first
quantitative explanation. Their argument has a clean two-stage structure:
...
\begin{enumerate}[label=(C\arabic*)]
  ...
  yields an LMC theorem for the practical single-Gaussian distributional model.
\end{enumerate}
...
\end{align*}
Since $\eta_2\le -\varepsilon$ and $|\eta_1|\le M$,
\[
  |\partial_{11}^2 A_{\rm G}(\eta)|+|\partial_{12}^2 A_{\rm G}(\eta)|
  \le
  \frac{1}{2\varepsilon}+\frac{M}{2\varepsilon^2},
\]
and, using symmetry of the Hessian,
\[
  |\partial_{21}^2 A_{\rm G}(\eta)|+|\partial_{22}^2 A_{\rm G}(\eta)|
  \le
  \frac{M}{2\varepsilon^2}
  +
  \frac{M^2}{2\varepsilon^3}
...
Therefore
\[
  \norm{\nabla^2 A_{\rm G}(\eta)}_{\mathrm{op}}
  \le
  \norm{\nabla^2 A_{\rm G}(\eta)}_{\infty}
  \le
  L_{\rm G}(M,\varepsilon),
\]
where $\norm{\cdot}_{\infty}$ is the maximum absolute row sum.
\end{proof}
...
\]
where $\zeta:[0,\infty)\to(0,\infty)$ is bounded and Lipschitz, and the data
stream
\[
...
\[
  \zeta_{\max}:=\sup_{t\in[0,T]}\zeta(t)<\infty.
\]
...
sides multiplied by $\zeta_{\max}$. Then every characteristic of the
time-inhomogeneous mean-field flow
\[
  \dot\theta(t)=\zeta(t)\,b(\theta(t),\rho_t)
\]
...
the additional prefactor $\zeta(t)$ absorbed by $\zeta_{\max}$ in the comparison
argument.
\end{proof}
...
is standard Euler-vs-ODE monotonicity: since $s_k\le \varepsilon\zeta_{\max}$ and
...
\begin{equation}
  \partial_t \rho_t
  +
  \nabla_\theta\cdot\bigl(\rho_t\,\zeta(t)b(\theta,\rho_t)\bigr)
  =
  0,
  \qquad
  \rho_{t=0}=\rho_0.
